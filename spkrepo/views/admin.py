...
from ..ext import db
from ..models import User, Build, Version, Package, Firmware, Architecture, Screenshot
    ...
    def action_activate(self, ids):
        try:
            ...
            for user in users:
                user.active = True
            self.session.commit()
            flash('User was successfully activated.' if len(users) == 1 else
                  '%d users were successfully activated.' % len(users))
...
def screenshot_namegen(obj, file_data):
    ...
    i = 1
    while os.path.exists(os.path.join(current_app.config['DATA_PATH'], obj.package.name, pattern % (i, ext))):
        i += 1
...
class VersionView(ModelView):
    ...
    # Hooks
    def on_model_delete(self, model):
        version_path = os.path.join(current_app.config['DATA_PATH'], model.package.name, str(model.version))
        if os.path.exists(version_path):
            shutil.rmtree(version_path)

    # View
    column_list = ('package', 'upstream_version', 'version', 'beta', 'service_dependencies',
                   'insert_date', 'all_builds_active', 'install_wizard', 'upgrade_wizard', 'startable')
    ...
    # Actions
    @action('activate', 'Activate', 'Are you sure you want to activate selected versions\' builds?')
    def action_activate(self, ids):
        try:
            ...
            for version in versions:
                for build in version.builds:
                    build.active = True
            self.session.commit()
            flash('Builds on version were successfully activated.' if len(versions) == 1 else
                  'Builds have been successfully activated for %d versions.' % len(versions))
        except Exception as e:  # pragma: no cover
            self.session.rollback()
            flash('Failed to activate versions\' builds. %s' % str(e), 'error')

    @action('deactivate', 'Deactivate', 'Are you sure you want to deactivate selected  versions\' builds?')
    def action_deactivate(self, ids):
        try:
            versions = get_query_for_ids(self.get_query(), self.model, ids).all()
            for version in versions:
...
class BuildView(ModelView):
    ...
    # View
    column_list = ('version.package', 'version.upstream_version', 'version.version', 'architectures', 'firmware',
                   'publisher', 'insert_date', 'active')
    column_labels = {'version.package': 'Package', 'version.upstream_version': 'Upstream Version',
    ...
    # Actions
    @action('activate', 'Activate', 'Are you sure you want to activate selected builds?')
    def action_activate(self, ids):
        try:
            ...
            for build in builds:
                build.active = True
            self.session.commit()
            flash('Build was successfully activated.' if len(builds) == 1 else
                  '%d builds were successfully activated.' % len(builds))
        except Exception as e:  # pragma: no cover
            self.session.rollback()
            flash('Failed to activate builds. %s' % str(e), 'error')

    @action('deactivate', 'Deactivate', 'Are you sure you want to deactivate selected builds?')
    def action_deactivate(self, ids):
        try:
            builds = get_query_for_ids(self.get_query(), self.model, ids).all()
            for build in builds:
