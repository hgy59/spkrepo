...
from spkrepo.models import Architecture, Firmware, Download
from spkrepo.tests.common import BaseTestCase, BuildFactory, DownloadFactory, PackageFactory
...
class CatalogTestCase(BaseTestCase):
    ...
    def test_missing_data_build(self):
        self.assert400(self.client.post(url_for('nas.catalog'), data=dict(arch='88f6281', language='enu')))

    def test_missing_data_language(self):
        self.assert400(self.client.post(url_for('nas.catalog'), data=dict(arch='88f6281', build='1594')))

    def test_wrong_data_arch(self):
        self.assert422(self.client.post(url_for('nas.catalog'),
    ...
    def test_stable_build_active_stable_5004(self):
        ...
        self.assertCatalogEntry(catalog['packages'][0], build, data, dict(arch='88f628x', build='5004'))
    ...
    def test_stable_build_active_stable_no_distributor(self):
        build = BuildFactory(active=True, version__report_url=None, version__distributor=None,
                             architectures=[Architecture.find('88f6281', syno=True)], firmware=Firmware.find(1594))
        db.session.commit()
        data = dict(arch='88f6281', build='1594', language='enu')
        response = self.client.post(url_for('nas.catalog'), data=data)
    ...
    def test_stable_build_active_stable_qstart(self):
        build = BuildFactory(active=True, version__report_url=None, version__license=None,
                             version__install_wizard=False, version__startable=None,
                             architectures=[Architecture.find('88f6281', syno=True)], firmware=Firmware.find(1594))
        db.session.commit()
        data = dict(arch='88f6281', build='1594', language='enu')
        response = self.client.post(url_for('nas.catalog'), data=data)
    ...
    def test_stable_build_active_stable_different_arch(self):
        BuildFactory(active=True, version__report_url=None, architectures=[Architecture.find('cedarview')],
                     firmware=Firmware.find(1594))
        db.session.commit()
        data = dict(arch='88f6281', build='1594', language='enu')
        response = self.client.post(url_for('nas.catalog'), data=data)
    ...
    def test_stable_build_active_not_stable(self):
        BuildFactory(active=True, architectures=[Architecture.find('88f6281', syno=True)], firmware=Firmware.find(1594))
        db.session.commit()
        data = dict(arch='88f6281', build='1594', language='enu')
        response = self.client.post(url_for('nas.catalog'), data=data)
        self.assert200(response)
    ...
    def test_not_stable_build_active_not_stable(self):
        build = BuildFactory(active=True, architectures=[Architecture.find('88f6281', syno=True)],
                             firmware=Firmware.find(1594))
        db.session.commit()
        data = dict(arch='88f6281', build='1594', language='enu', package_update_channel='beta')
        response = self.client.post(url_for('nas.catalog'), data=data)
...
class DownloadTestCase(BaseTestCase):
    ...
    def test_wrong_build(self):
        architecture = Architecture.find('88f6281', syno=True)
        build = BuildFactory(active=False, version__report_url=None,
                             architectures=[architecture], firmware=Firmware.find(1594))
        db.session.commit()
        self.assertEqual(Download.query.count(), 0)
        response = self.client.get(url_for('nas.download', architecture_id=architecture.id,
                                           firmware_build=4458, build_id=build.id + 1))
    ...
    def test_wrong_architecture(self):
        ...
        db.session.commit()
        self.assertEqual(Download.query.count(), 0)
        response = self.client.get(url_for('nas.download', architecture_id=10,
                                           firmware_build=4458, build_id=build.id))
        self.assert404(response)
        self.assertEqual(Download.query.count(), 0)
